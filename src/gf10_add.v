...
// t27/rtl_gen/gf10_add.v
// GoldenFloat10 Addition Unit -- [S(1) | E(3) | M(6)], bias 2^(E-1)-1.
...
module gf10_add (
    ...
    reg [5:0] big_m, small_m;
    reg [9:0] big_ext, small_ext, shifted;   // {1, mant, 3 guard bits}
    reg [3:0]  shamt;
    reg            sticky_shift;
    reg [10:0] sum_m;                          // room for add carry
    reg signed [4:0] rexp;
    reg [5:0]  mant_out;
    reg [6:0]   mant_rounded;                    // M+1 bits, catches overflow
    ...
    always @(*) begin
        ...
        if (is_nan_a || is_nan_b)
            ...
            result = sign_a ? 10'h3C0 : 10'h1C0;
        else if (is_inf_b)
            result = sign_b ? 10'h3C0 : 10'h1C0;
            ...
            if (shamt >= 10)
                ...
                sticky_shift = |(small_ext & ((10'd1 << shamt) - 10'd1));
            end
            ...
            if (sum_m == 0) begin
                ...
                mant_rounded = {1'b0, mant_out} + (round_up ? 7'd1 : 7'd0);
                if (mant_rounded[6]) begin
                    rexp     = rexp + 5'sd1;
                    mant_out = 6'd0;
                end else begin
                    mant_out = mant_rounded[5:0];
                end

                if (rexp < 0)
                    result = result_sign ? 10'h200 : 10'h0;
                else if (rexp >= EXP_MAX_S)
                    result = result_sign ? 10'h3C0 : 10'h1C0;
                else if (rexp == 0 && mant_out == 6'd0)
                    result = result_sign ? 10'h200 : 10'h0;
                else
                    result = {result_sign, rexp[2:0], mant_out};
            end
        end
    end
