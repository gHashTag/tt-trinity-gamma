...
// bitnet_encoder_v2.v — corrected next-gen variant of bitnet_encoder.
// Apache-2.0
//
// FIX (loop 129): the shipped bitnet_encoder addresses Layer-1 weights with a
// 10-bit neuron_base, but the 32 neurons span k*64 = 0..1984 (11-bit), so neurons
// 16..31 aliased 0..15 (h1[k]==h1[k-16]) -- only 16 distinct neurons were realized.
// This v2 widens neuron_base AND w_gen's addr/hash to 11 bits (mixing all 11 bits,
// so addr 0 and 1024 no longer collide), giving the full 0..2047 weight-address
// range and 32 distinct hidden neurons. SEPARATE module: the frozen bitnet_encoder
// is left as taped out (Gamma/Euler silicon). See docs/FULL_DESIGN_LINT_TRIAGE.md
// (Corona) and test/tb_bitnet_v2.v.
//
